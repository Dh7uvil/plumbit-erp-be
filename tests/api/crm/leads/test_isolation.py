...
@pytest.mark.asyncio
async def test_lead_tenant_isolation(client: AsyncClient) -> None:
    ...
    created = await client.post(
        "/api/v1/leads",
        headers=headers_a,
        json={"first_name": "Iso", "last_name": uuid4().hex[:8]},
    )
    assert created.status_code == 201, created.text
    lead_id = created.json()["data"]["id"]
    fetched = await client.get(f"/api/v1/leads/{lead_id}", headers=headers_b)
