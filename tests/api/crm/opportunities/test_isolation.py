"""Tenant isolation for opportunities."""
...
@pytest.mark.asyncio
async def test_opportunity_tenant_isolation(client: AsyncClient) -> None:
    ...
    created = await client.post(
        ...
        headers=headers_a,
        json={"name": f"Iso {uuid4().hex[:8]}"},
    )
    assert created.status_code == 201, created.text
    opportunity_id = created.json()["data"]["id"]
    fetched = await client.get(f"/api/v1/opportunities/{opportunity_id}", headers=headers_b)
