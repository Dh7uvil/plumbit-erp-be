"""Tenant isolation for lost reasons."""
...
@pytest.mark.asyncio
async def test_lost_reason_tenant_isolation(client: AsyncClient) -> None:
    ...
    created = await client.post(
        "/api/v1/lost-reasons",
        headers=headers_a,
        json={"name": f"Reason {uuid4().hex[:8]}"},
    )
    assert created.status_code == 201, created.text
    lost_reason_id = created.json()["data"]["id"]
    fetched = await client.get(f"/api/v1/lost-reasons/{lost_reason_id}", headers=headers_b)
