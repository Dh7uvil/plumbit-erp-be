...
def next_status(current: StockDocumentStatus, action: str) -> StockDocumentStatus:
    ...
    if target is None:
        raise InvalidStatusTransitionError(
            f"Cannot {action} a landed cost in {current.value} status"
        )
    return target
...
def assert_editable(status: StockDocumentStatus) -> None:
    if status not in _EDITABLE:
        raise InvalidStatusTransitionError("Only draft landed costs can be edited")
