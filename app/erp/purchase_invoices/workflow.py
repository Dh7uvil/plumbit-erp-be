"""Purchase invoice status machine."""
...
def next_status(current: InvoiceDocumentStatus, action: str) -> InvoiceDocumentStatus:
    ...
    if target is None:
        raise InvalidStatusTransitionError(
            f"Cannot {action} a purchase invoice in {current.value} status"
        )
    return target
...
def assert_editable(status: InvoiceDocumentStatus) -> None:
    if status not in _EDITABLE:
        raise InvalidStatusTransitionError("Only draft purchase invoices can be edited")
