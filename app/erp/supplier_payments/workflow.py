"""Supplier payment status machine."""
...
def next_status(current: InvoiceDocumentStatus, action: str) -> InvoiceDocumentStatus:
    ...
    if target is None:
        raise InvalidStatusTransitionError(
            f"Cannot {action} a supplier payment in {current.value} status"
        )
    return target
...
def assert_editable(status: InvoiceDocumentStatus) -> None:
    if status not in _EDITABLE:
        raise InvalidStatusTransitionError("Only draft supplier payments can be edited")
